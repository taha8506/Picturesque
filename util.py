...
def tone_map(img, tmo_name, **kwargs):
    TMO_DICT = {
        'exposure': lambda: cv2.createSimpleToneMapper(kwargs['gamma']),
        'reinhard': lambda: cv2.createTonemapReinhard(kwargs['gamma'], kwargs['intensity'], kwargs['light_adapt'], kwargs['color_adapt']),
        'mantiuk': lambda: cv2.createTonemapMantiuk(kwargs['gamma'], kwargs['scale'], kwargs['saturation']),
        'drago': lambda: cv2.createTonemapDrago(kwargs['gamma'], kwargs['saturation'], kwargs['bias']),
        'durand': lambda: cv2.createTonemapDurand(kwargs['gamma'], kwargs['contrast'], kwargs['saturation'], kwargs['sigma_space'], kwargs['sigma_color'])
    }
    return TMO_DICT[tmo_name]().process(img)
